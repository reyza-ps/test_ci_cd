class RegisterToPartyJob
  ...
  def perform(token)
    current_access_token = Partner::AccessToken.find_by(token:)
    ...
    Partner::AccessToken.update_to_token_c(access_token: current_access_token,
                                           ocpi_credential: registration_result.their_credential)
  end
end
