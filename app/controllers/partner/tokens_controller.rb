class Partner::TokensController < Doorkeeper::TokensController
  ...
  before_action :get_application, only: %i[create ocpi_connect ocpi_request ocpi_disconnect]
  ...
  def ocpi_request
    ...
    if response.status == :ok
      body[:id] = response.token.resource_owner_id unless nil
      current_access_token = Partner::AccessToken.create_relations(access_grant_token: params[:code],
  ...
  def ocpi_disconnect
    ...
    result = Ocpi::Application::Credentials::Services::PushUnregister.new(
      credential_id: current_access_token.ocpi_credentials_id, extras:
    ).call
    Partner::AccessToken.reset_ocpi_config!(current_access_token:) if result.destroyed? == true
