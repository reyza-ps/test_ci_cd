class Admin::Partner::OauthApplicationsController < AdminController
  ...
  def approve
    if @application.present?
      ...
      redirect_to admin_partner_oauth_application_path(partner_id: @partner.id, id: @application.id),
                  flash: { success: 'Application successfully approved.' }
    else
  ...
  def validate
    ...
    flash[:error] = I18n.t(:notice, scope: %i[doorkeeper flash applications ocpi invalid]) + ' => ' + e.to_s
    redirect_to admin_partner_oauth_application_path(partner_id: @partner.id, id: @application.id)
  end
  ...
  def receive_push
    if @application.update(receive_push_model: params[:receive_push_model])
      ...
      redirect_to admin_partner_oauth_application_path(partner_id: @partner.id, id: @application.id),
                  flash: { success: "Push Model successfully #{status}" }
    else
      redirect_to admin_partner_oauth_application_path(partner_id: @partner.id, id: @application.id),
  ...
  def application_params
    params.require(:partner_application).permit(:name, :redirect_uri, :scopes, :confidential, :payment_gateway_webhook,
